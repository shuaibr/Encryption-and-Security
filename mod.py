...
def is_relatively_prime(a,b):
    if not isinstance(a, int) or not isinstance(b, int):
        ...
        return True if gcd(a,b) is 1 else False 
...
def has_mul_inv(a,m):
        if not isinstance(a, int):
            ...
            return 'Error (has_mul_inv): Invalid input mod'
        else:
            a = a%m 
            for i in range(m):
                if ((a*i)%m==1):
